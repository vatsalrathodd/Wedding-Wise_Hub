...
if (isset($_POST['submit']) || isset($_FILES['file'])) {
    $gallery->title = my_ob_clean($_POST['title']);
    $gallery->caption = my_ob_clean($_POST['caption']);
    $gallery->alternate_text = my_ob_clean($_POST['alternate_text']);
    $gallery->description = my_ob_clean($_POST['description']);
    $gallery->booking_id = my_ob_clean($_POST['booking_id']);
            ...
            <div class="col-lg-8 offset-2 pl-3 pb-3 box-shadow mt-4 pt-3">
                ...
                <form method="post" action="" enctype="multipart/form-data">
                    <div class="form-group">
                        ...
                        <select class="custom-select form-control" id="booking_id" name="booking_id">
                            <?php foreach ($booking as $booking_user) : ?>
                                <?php $isSelected = ($booking_user->booking_id == $gallery->booking_id) ? 'selected' : ''; ?>
                                <option value="<?= $booking_user->booking_id; ?>" <?= $isSelected; ?>>
                                    <?= $booking_user->bride . ' + ' . $booking_user->groom; ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="alternate_text" class="form-control" placeholder="Enter text">
                    </div>
                    <div class="form-group">
